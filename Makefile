...
PROJECT_NAME=trepr
STACK_NAME=trepr-ericof-tangrama-com-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://trepr.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t ericof/trepr-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t ericof/trepr-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name trepr-frontend-acceptance --link trepr-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ericof/trepr-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name trepr-backend-acceptance -d ericof/trepr-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop trepr-frontend-acceptance
	@docker stop trepr-backend-acceptance
